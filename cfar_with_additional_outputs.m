...
% PARAM: advanced_config, struct, struct('noise_floor', -50, 'min_snr', 10, 'enable_filter', true)
    ...
    advanced_config = getParam(params, 'advanced_config', struct('noise_floor', -50, 'min_snr', 10, 'enable_filter', true));
    ...
    % 应用噪声底限过滤（使用advanced_config）
    if apply_log && isfield(advanced_config, 'noise_floor')
        noise_floor = advanced_config.noise_floor;
        magnitude(magnitude < noise_floor) = noise_floor;
    end
    ...
    for col = 1:cols
        for row = 1:rows
            ...
            if magnitude(row, col) > threshold
                % 如果启用了SNR过滤，额外检查SNR
                if isfield(advanced_config, 'enable_filter') && advanced_config.enable_filter
                    if isfield(advanced_config, 'min_snr')
                        snr = magnitude(row, col) - mean_value;
                        if snr >= advanced_config.min_snr
                            detected(row, col) = 1;
                        end
                    else
                        detected(row, col) = 1;
